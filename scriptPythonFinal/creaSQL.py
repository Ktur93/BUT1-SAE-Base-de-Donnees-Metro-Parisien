...
def trier_lignes_metro(dico):
    def cle_tri(item):
        numero = item[0][1]  # ('METRO', '9') -> '9'
        
        import re
        match = re.match(r"(\d+)(.*)", numero)
        
        base = int(match.group(1))      # partie numérique
        suffixe = match.group(2)        # ex: '', 'bis', 'b'
        
        return (base, suffixe)
    
    return dict(sorted(dico.items(), key=cle_tri))
...
def construire_stations(rows: list[dict]) -> dict[int, dict]:
    ...
    for row in rows:
        ...
        print("acess", row.get("accessibilite_pmr"))
        if (str(row.get("accessibilite_pmr")) == '3.0' or str(row.get("accessibilite_pmr")) == '4.0'):
            pmr = True
        else:
            pmr = False
...
def generer_sql(lignes: dict, stations: dict, dessert: list, chemin_out: str):
    ...
    lignes_triees = trier_lignes_metro(lignes).items()  
    ...
    for cle, data in lignes_triees:
        ...
        lines.append(
            f"INSERT INTO ligne (ligne_id, nom_ligne, couleur_hex) \n    VALUES ("
            f"{lid}, "
...
def main():
    ...
    parser.add_argument(
        "--csv",
        default="station_metro_paris_cp_acc.csv",
        help="Chemin vers le fichier CSV source"
    )
    parser.add_argument(
        "--out",
        default="inserts_metro_idf_final-2.sql",
    ...
    lignes = trier_lignes_metro(lignes)
...
def construireDicoStationIdStation() :
    dico = {}
    for key, value in construire_stations(lire_csv("station_metro_paris_cp.csv")).items() :
        v = unidecode(value['nom_station'].replace(" ", "").replace("-", "").replace("'", "").lower())
        dico[v.replace(" ", "").replace("-", "").replace("'", "")] = key
    return dico
def construireDicoStationId() :
    dico = {}
    for key, value in construire_stations(lire_csv("station_metro_paris_cp.csv")).items() :
        v = unidecode(str(value['nom_station']).replace(" ", "").replace("-", "").replace("'", "").lower())
        dico[key] = v
    return dico

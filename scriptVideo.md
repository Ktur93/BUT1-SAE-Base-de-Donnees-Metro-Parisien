...
Pour alimenter notre base de données, nous avons recherché sur Internet des fichiers CSV contenant des informations sur les stations.

Ensuite, nous avons créé plusieurs scripts Python pour transformer ces données en scripts d’insertion SQL.

Le premier script permet de générer les lignes, les stations et les dessertes.
Le second permet de générer les trajets sur chaque ligne.

Nous avons choisi de générer un nombre important de trajets, avec des horaires cohérents et une petite part d’aléatoire pour simuler les variations de circulation.

Cela nous permet d’obtenir un jeu de données suffisamment grand et exploitable pour l’analyse, ce qui correspond aussi à une exigence de la SAE.
...
ou encore calculer le temps moyen entre deux stations sur une même ligne.
